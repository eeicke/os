...
#	dd seek=2048 conv=sync if=/dev/zero of=$(DISK_IMG) bs=512 count=6
...
$(KERNEL_IMAGE): $(BIN_DIR) $(KERNEL_SRC_DIR)/*.asm $(KERNEL_SRC_DIR)/*.c $(KERNEL_SRC_DIR)/*.h
	...
	ld -melf_i386 -T$(KERNEL_LINKER_SCRIPT) $(KERNEL_START_OBJECT_FILE) $(KERNEL_MAIN_ELF_FILE) $(KERNEL_DISPLAY_ELF_FILE) $(KERNEL_MEMMAP_ELF_FILE) $(KERNEL_UITOA_ELF_FILE) $(KERNEL_ATA_ELF_FILE)  -o $(KERNEL_ELF_FILE)
