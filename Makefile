...
KERNEL_MAIN_ELF_FILE = $(BIN_DIR)/main.elf
...
QEMU = qemu-system-x86_64 -s -drive format=raw,file=$(DISK_IMG)
QEMU_DEBUG = qemu-system-x86_64 -s -S -drive format=raw,file=$(DISK_IMG)

build: $(BOOT_IMG) $(KERNEL_IMAGE)
	dd if=$(BOOT_IMG) of=$(DISK_IMG)
	dd seek=1 conv=sync if=$(KERNEL_IMAGE) of=$(DISK_IMG) bs=512 count=3	
...
$(KERNEL_IMAGE): $(BIN_DIR) $(KERNEL_SRC_DIR)/*.asm $(KERNEL_SRC_DIR)/*.c
	...
	ld -melf_i386 -T$(KERNEL_LINKER_SCRIPT) $(KERNEL_START_OBJECT_FILE) $(KERNEL_MAIN_ELF_FILE) -o $(KERNEL_ELF_FILE)
